...
async def create_appeal(
    ...
    try:
        ...
        AuditTrailService.log_to_database(
            ...
            details={
                "reason": reason,
                "decision_explanation_keys": list(decision_explanation.keys())
...
async def update_appeal_status(
    ...
    try:
        ...
        if dataset_id and audit_id:
            AuditTrailService.log_to_database(
                dataset_id=dataset_id,
                audit_id=audit_id,
                appeal_id=appeal_id,
                actor=reviewer_id,
                action=f"appeal_status_changed_to_{new_status}",
                entity_type="appeal",
                entity_id=appeal_id,
                details={
...
async def get_appeal_status(appeal_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": str(e), "message": "Error retrieving appeal status"}


async def generate_appeal_confirmation(appeal_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Generating confirmation for appeal {appeal_data.get('appeal_id')}"
        )
        ...
        return {"error": str(e), "message": "Error generating confirmation"}
