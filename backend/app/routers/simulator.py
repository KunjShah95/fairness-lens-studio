...
async def get_counterfactuals(request: CounterfactualRequest, db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        logger.info(f"[Simulator] Generating counterfactuals for audit {request.audit_id}")
        ...
        logger.info(f"[Simulator] Generated {result.get('num_counterfactuals', 0)} counterfactuals")
        ...
        logger.error(f"[Simulator] Error generating counterfactuals: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/population-impact")
async def get_population_impact(request: PopulationImpactRequest, db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        logger.info(f"[Simulator] Estimating population impact for audit {request.audit_id}")
        ...
        logger.info(f"[Simulator] Impact estimate: {result.get('total_newly_approved', 0)} newly approved")
        ...
        logger.error(f"[Simulator] Error estimating population impact: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/scenario")
async def model_what_if_scenario(request: ScenarioRequest, db: Session = Depends(get_db)):
    """
