...
async def get_audit_trail(
    entity_id: str, dataset_id: Optional[str] = Query(None)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        history = audit_trail_service.AuditTrailService.get_audit_history(
...
async def list_all_regulations(domain: str = None) -> Dict[str, Any]:
    """List all applicable regulations, optionally filtered by domain."""
    try:
        from app.services.compliance_service import ComplianceFramework

        logger.info(f"Listing regulations for domain: {domain}")

        all_regulations = []

        # Collect standards from framework
        for code, standard in ComplianceFramework.STANDARDS.items():
            if domain is None or domain == "all" or standard.applies_to_domain(domain):
        ...
        return {"regulations": all_regulations, "count": len(all_regulations)}
...
async def map_metrics_to_regulations(
    metrics: Dict[str, Any], domain: str = "general"
) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Mapping metrics to regulations for domain: {domain}")

        # Get relevant regulations
        applicable_regs = []
        for code, standard in ComplianceFramework.STANDARDS.items():
            if standard.applies_to_domain(domain):
                applicable_regs.append(standard)

        # Check each metric against applicable regulations
        mapped = []
        for reg in applicable_regs:
            ...
            if "fairness_score" in metrics:
                ...
                if score < 60:
                    issues.append(f"Fairness score {score} below recommended threshold")
                    compliant = False

            # Check specific metrics
            if "demographic_parity" in metrics:
                dp = metrics.get("demographic_parity", 1.0)
                if dp < 0.8:
                    issues.append(f"Demographic parity {dp} below 0.8 threshold")
                    compliant = False
        ...
        return {"domain": domain, "mapped_regulations": mapped}
...
async def check_compliance(
    audit_id: str, jurisdiction: str = Query("general"), domain: str = Query("general")
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        report = compliance_service.ComplianceChecker.check_compliance(
            audit_results=audit_dict, jurisdiction=jurisdiction, domain=domain
        )
...
async def assign_to_committee(
    appeal_id: str, dataset_id: str, description: str = ""
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        committee_id = committee_service.CommitteeService.create_default_committee(
            dataset_id
        )

        # Assign to committee
        result = workflow.assign_appeal_to_committee(
            appeal_id=appeal_id, committee_id=committee_id, description=description
        )
...
async def vote_on_appeal(
    ...
    try:
        ...
        result = workflow.vote_on_appeal(
            appeal_id=appeal_id, reviewer_id=reviewer_id, decision=decision
        )
...
async def evaluate_appeal_escalation(
    appeal_id: str, dataset_id: str, audit_id: str, appellant_reason: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Evaluate escalation
        recommendation = (
            await committee_service.CommitteeService.evaluate_appeal_for_committee(
                appeal_id=appeal_id,
        ...
        logger.info(
            f"Escalation recommendation: {recommendation['should_escalate_to_committee']}"
        )
